...
export const useOrdersStore = defineStore('orders', () => {
  ...
  async function fetchOrders() {
    try {
      ...
      // Сначала загружаем базовые данные заказов
      ...
      // Теперь загружаем полные данные для каждого заказа отдельно
      const fullOrdersPromises = response.data.data.map(async (order) => {
        try {
          console.log(`[ordersStore] Loading full data for order ${order.id}`);
          const fullOrderResponse = await api.get(`/orders/${order.id}`, {
            params: {
              populate: {
                client: true,
                car: true,
          ...
          return transformOrderData(fullOrderResponse.data);
        } catch (error) {
          console.error(`[ordersStore] Error loading full data for order ${order.id}:`, error);
          // Возвращаем базовые данные если не удалось загрузить полные
          return transformOrderData(order);
        }
      });

      const transformedOrders = await Promise.all(fullOrdersPromises);
      console.log('[ordersStore] First transformed order works:', transformedOrders[0]?.works);
      console.log('[ordersStore] First transformed order parts:', transformedOrders[0]?.parts);
