...
const sharePDF = async () => {
  ...
  try {
    ...
    const file = new File([props.pdfData.blob], props.pdfData.filename, { type: 'application/pdf' });
    const strapiUrl = await uploadImage(file);

    console.log('Загруженный PDF URL:', strapiUrl); // Логируем URL после загрузки
    ...
    // Логируем сформированную публичную ссылку
    console.log('Публичная ссылка на PDF:', mainDomainUrl);
...
const downloadPDF = () => {
  ...
  notifySuccess('PDF успешно скачан!');
};
</script>
