<template>
  <div v-if="isOpen" class="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
    <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-xl max-w-md w-full mx-4">
      <h2 class="text-xl font-semibold mb-4 text-gray-900 dark:text-white">Ссылка для поделиться заказом</h2>

      <div class="mb-4">
        <p class="text-sm text-gray-600 dark:text-gray-300 mb-2">
          Заказ-наряд доступен по публичной ссылке. Ссылка для копирования:
        </p>
...
const openLink = () => {
  window.open(fullShareUrl.value, '_blank')
}
</script>
