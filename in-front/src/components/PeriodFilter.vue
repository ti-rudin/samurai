          ...
          <div class="flex flex-col lg:flex-row gap-3 lg:items-center">
            <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 flex-1">
              <input
                ...
                type="date"
                class="w-full sm:w-48 lg:w-56 h-11 px-3 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition"
                @input="$emit('update:customStartDate', $event.target.value)"
                @change="$emit('custom-date-change')"
              />
              <span class="text-gray-500 text-center sm:text-left">—</span>
              <input
                :value="customEndDate"
                type="date"
                class="w-full sm:w-48 lg:w-56 h-11 px-3 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition"
